...
通过使用一个哈希表map将标识符和值绑定到一块。为此要专门设置一个环境Environment。其工作原理是，将标识符及其值写入哈希表，用的时候再查出来。
```go
...
// 获取Object对象
func (e *Environment) Get(name string) (Object, bool) {//获取值
	obj, ok := e.store[name]
	return obj, ok
}

// 设置Object对象
func (e *Environment) Set(name string, val Object) Object {//写入值
	e.store[name] = val
	return val
}
