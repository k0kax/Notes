...
此处采用将Pairs定义为`map[string]Object`，然后使用`*object.String`的Value作为键。因此需要一个哈希函数，来为`*object.String`生成一个能够比较的哈希键。如果另一个`*object.String`具有相同的Value，那么两者的哈希值应该相
等。`*object.Integer`和`*object.Boolean`也是如此。但是`*object.String`的哈希键一定不能等于`*object.Integer`或`*object.Boolean`的哈希键。不同类型的哈希键必须不同。
