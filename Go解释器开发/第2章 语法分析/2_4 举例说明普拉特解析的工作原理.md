...
```
中缀表达式，天生适合用来解析。要完成一次中缀解析，首先需要构造一个中缀结构体。
总解析函数
```go
...
parseExpression是普拉特解析器的核心之一，一个正确的数学表达式如`1+2*3`。它首先根据目前词法单元curToken的优先级，匹配对应的前缀解析函数，没有的话就不执行。有的话就执行，前缀解析函数会构造一个前缀结构体，包括Token和操作符分别是当前的token和它的字面量。之后，就会后移一位。这个结构体的右半部分



好了现在有左半部分了，接着就需要右半部分的符号和表达式了。首先，右边第一位也就是peekToken必须是个运算符如加减乘除了，不能为分号，且优先级必须大于当前的优先级，否则没法向右进行。紧接着，就需要调用中缀解析函数了。
