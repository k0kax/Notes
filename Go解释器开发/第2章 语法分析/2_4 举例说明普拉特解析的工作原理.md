...
![](https://raw.githubusercontent.com/k0kax/PicGo/main/images20251209105740674.png)
第三步
执行解析函数`leftExp := prefix()`，执行解析函数，即`leftExp=1`。此时
![](https://raw.githubusercontent.com/k0kax/PicGo/main/images20251209105740674.png)
第四步
进入循环判断，循环的条件是`!p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence()`。即当前的下一个词法单元curToken类型不是分号(;)和当前优先级小于下一个词法单元的优先级
