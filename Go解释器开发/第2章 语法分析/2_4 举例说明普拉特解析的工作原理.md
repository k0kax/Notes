...
## 第三步 递归解析2 * 3 
继续调用parseExpression()函数，此时precedence=SUM(3)，token如下
![](https://raw.githubusercontent.com/k0kax/PicGo/main/images20251209121531012.png)
...
func (p *Parser) parseInfixExpression(left ast.Expression) ast.Expression {
	...
	p.nextToken()                                    
	expression_2.Right = p.parseExpression(precedence) 
	return expression_2
}
```
 初始化中缀节点
...
```
此时`leftExp_2 = 2 * 3`的节点  。进行循环检查，peekToken=分号中止循环，返回leftExp_2=2 * 3
