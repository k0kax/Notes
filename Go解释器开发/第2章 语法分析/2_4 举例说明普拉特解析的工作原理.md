...
parseExpression是普拉特解析器的核心之一，一个正确的数学表达式如`1+2*3`。它首先根据目前词法单元curToken的优先级，匹配对应的前缀解析函数prefix，没有的话就不执行。有的话就执行，前缀解析函数，最后会返回leftExp作为树的右半部分之后。好了现在有左半部分了，接着就需要右半部分的符号和表达式了。首先，右边第一位也就是peekToken必须是个运算符如加减乘除了，不能为分号，且优先级必须大于当前的优先级，否则没法向右进行。这是普拉特解析中处理运算符优先级的核心，确保高优先级运算符先结合，从而正确构建符合运算规则的 AST（比如让* 先于+结合）。紧接着，就进入循环了。第一步，需要匹配当前token的解析函数infix，如果没有，就直接返回左半部分，解析结束，表达式只有左边这一点。infix不空的话，就继续往下执行。紧接着token移动，需要操作下一个操作符了。

好了，现在要讲讲中缀解析函数了。中缀解析式接受左半部分leftExp，构成一个结构体expression。它的左边还是leftExp。新符号也写进它。接着获取当前的新符号的优先级。向下移动token,再次调用parseExpression函数按照当前的优先级，写道这个结构体的右边。
