...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		...
		leftExp = infix(leftExp) //递归入口：处理当前运算符+右侧表达式 对应2.2
	}
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		...
		leftExp_2 = infix(leftExp_2)//递归入口：处理当前运算符*右侧表达式 带数字后缀是为区分层次
	}
...
// 中缀解析函数
func (p *Parser) parseInfixExpression(left ast.Expression) ast.Expression {
	expression_2 := &ast.InfixExpression{//带数字后缀是为区分层次
		Token:    p.curToken,         
		Operator: p.curToken.Literal, 
		Left:     left,               
	}
	...
	precedence := p.curPrecedence()                  
	p.nextToken()                                    
	expression_2.Right = p.parseExpression(precedence) //返回值对应3.3 带数字后缀是为区分层次
	return expression_2
}
```
 初始化中缀节点
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		...
		p.nextToken()
		leftExp_3 = infix(leftExp_3)//带数字后缀是为区分层次
	}
