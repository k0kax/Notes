...
- **program**（程序路径）—— 启动调试器时要运行的可执行文件或目标文件。
	- 假设文件在目录的xx文件夹下的yy.xx文件，则为`${workspaceFolder}/xx/yy`.xx
...
| `go test evaluator_test.go -test.run TestLetStatements`      | `"args": ["-test.run", "TestLetStatements"]`                                                       | 执行test文件的TestLetStatements函数 |
